...
func readFiles() {
	dat, err := os.ReadFile(`E:\Program_Files\progjar\encipher2`)
	check(err)
	fmt.Print(string(dat))

	f, err := os.Open(`E:\Program_Files\progjar\encipher2`)
	check(err)
...
func writeFiles() {
	...
	err := os.WriteFile(`E:\Program_Files\progjar\encipher2`, d1, 0644)
	check(err)

	f, err := os.Create(`E:\Program_Files\progjar\encipher2`)
	check(err)
